...
|------|------|
| フロントエンド | Streamlit |
| LLM | vLLM (Qwen3.5-35B-A3B、OpenAI互換API) |
...
| `VLLM_MODEL_NAME` | `Qwen/Qwen3.5-35B-A3B` | モデル名（HuggingFace形式） |
...
`Qwen/Qwen3.5-35B-A3B` モデルをOpenAI互換APIで提供するサーバー。
...
必要に応じて量子化をしてメモリ使用量を削減すると良いです。

```bash
# fp8 量子化で起動（VRAM削減）
vllm serve "$VLLM_MODEL_NAME" --quantization fp8 --host 0.0.0.0 --port 8000
```

より軽量なモデルを使いたい場合は `.env` の `VLLM_MODEL_NAME` を変更してください。

#### RAGサーバー

ビデオ解析データを提供するサーバー（`POST /query`）。RAGサーバーの起動方法は `rag-server/README.md` を参照してください。
